...
### 0. 命盘存档同步 API（🔥 紧急 - 导致同步失败）
**路径**: `/api/astrology/archives/*`

- [ ] `POST /api/astrology/archives` - 创建命盘存档
  - 请求体：`{ chart: object, name: string, relationship_type: string, ... }`
  - 响应：`{ success: boolean, data: { archive_id: string } }`
  - **严重性**：缺少此接口导致用户命盘无法保存到云端，跨设备同步失败（出现 404 错误）。

- [ ] `GET /api/astrology/archives` - 获取命盘存档列表
  - 请求参数：`relationship_type`, `limit`, `offset`
  - 响应：`{ success: boolean, data: { archives: [], total: number } }`
  - **严重性**：缺少此接口导致用户在更换设备后看不到任何历史命盘。

- [ ] `DELETE /api/astrology/archives/:id` - 删除命盘存档
  - 响应：`{ success: boolean }`
