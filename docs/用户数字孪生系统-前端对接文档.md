# 用户数字孪生系统 - 前端对接文档

**文档版本**：v1.0  
**创建日期**：2026-01-31  
**后端状态**：✅ 已完成  
**API 文档地址**：`http://localhost:3000/api-docs`

---

## 📋 目录

1. [概述](#概述)
2. [基础信息](#基础信息)
3. [认证说明](#认证说明)
4. [API 接口列表](#api-接口列表)
5. [数据类型定义](#数据类型定义)
6. [使用示例](#使用示例)
7. [注意事项](#注意事项)
8. [常见问题](#常见问题)

---

## 概述

用户数字孪生系统提供了一套完整的用户上下文管理 API，包括：

- **命主名刺**：用户的显性层信息（MBTI、职业、现状、愿景等）
- **资料完整度**：计算和展示用户资料完整度，支持奖励机制
- **隐性信息**：系统推断的用户特征（角色、兴趣、风险偏好等）
- **生辰信息同步**：自动同步生辰信息到用户上下文

---

## 基础信息

### Base URL

```
开发环境: http://localhost:3000/api
生产环境: https://api.yourdomain.com/api
```

### 响应格式

所有 API 响应统一使用以下格式：

```typescript
// 成功响应
{
  success: true,
  data: { ... },        // 响应数据
  message?: string      // 可选的成功消息
}

// 错误响应
{
  success: false,
  error: string,        // 错误代码
  message?: string      // 错误描述
}
```

### HTTP 状态码

- `200` - 成功
- `400` - 参数错误
- `401` - 未认证
- `404` - 资源不存在
- `500` - 服务器错误

---

## 认证说明

### 认证方式

所有接口都需要在请求头中携带 JWT Token：

```
Authorization: Bearer <token>
```

### 获取 Token

通过登录接口获取 Token：

```typescript
// POST /api/auth/login
const response = await fetch('/api/auth/login', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    email: 'user@example.com',
    password: 'password123'
  })
});

const { data } = await response.json();
const token = data.token;
```

### Token 存储

建议将 Token 存储在：
- **Web**: `localStorage` 或 `sessionStorage`
- **移动端**: 安全存储（Keychain/Keystore）

---

## API 接口列表

### 1. 获取命主名刺

**接口**：`GET /api/user/destiny-card`

**请求头**：
```
Authorization: Bearer <token>
```

**响应示例**：
```json
{
  "success": true,
  "data": {
    "mbti": "INTJ",
    "currentStatus": "刚被裁员，想创业",
    "identity": "紫微七杀·化杀为权",
    "profession": "独立开发者",
    "wishes": ["财务自由", "家庭和睦"],
    "energyLevel": "balanced",
    "completeness": 60,
    "lastUpdated": "2026-01-31T12:00:00.000Z"
  }
}
```

**TypeScript 类型**：
```typescript
interface DestinyCard {
  mbti?: string;
  currentStatus?: string;
  identity?: string;
  profession?: string;
  wishes?: string[];
  energyLevel?: 'strong' | 'weak' | 'balanced';
  completeness: number;        // 0-100
  lastUpdated: string;          // ISO 8601 日期字符串
}
```

---

### 2. 更新命主名刺

**接口**：`PUT /api/user/destiny-card`

**请求头**：
```
Authorization: Bearer <token>
Content-Type: application/json
```

**请求体**（所有字段可选）：
```json
{
  "mbti": "INTJ",
  "profession": "软件工程师",
  "currentStatus": "刚被裁员，想创业",
  "wishes": ["财务自由", "家庭和睦"],
  "energyLevel": "balanced",
  "identity": "紫微七杀·化杀为权"
}
```

**响应示例**：
```json
{
  "success": true,
  "message": "命主名刺已更新",
  "data": {
    "mbti": "INTJ",
    "profession": "软件工程师",
    "currentStatus": "刚被裁员，想创业",
    "wishes": ["财务自由", "家庭和睦"],
    "energyLevel": "balanced",
    "identity": "紫微七杀·化杀为权",
    "completeness": 60,
    "lastUpdated": "2026-01-31T12:00:00.000Z",
    "events": [
      {
        "type": "COIN_GRANTED",
        "coins": 5,
        "reason": "完善MBTI信息",
        "field": "mbti"
      },
      {
        "type": "THRESHOLD_REACHED",
        "coins": 10,
        "reason": "资料完整度达到30%",
        "threshold": 30
      }
    ]
  }
}
```

**重要说明**：
- 更新后可能返回 `events` 数组，包含奖励事件
- 前端应检查 `events` 并显示奖励动画
- 奖励发放具有幂等性，重复更新不会重复发放

**TypeScript 类型**：
```typescript
interface UpdateDestinyCardRequest {
  mbti?: string;
  profession?: string;
  currentStatus?: string;
  wishes?: string[];
  energyLevel?: 'strong' | 'weak' | 'balanced';
  identity?: string;
}

interface RewardEvent {
  type: 'COIN_GRANTED' | 'THRESHOLD_REACHED' | 'COMPLETENESS_INCREASED';
  coins?: number;
  reason: string;
  field?: string;
  threshold?: number;
}

interface UpdateDestinyCardResponse {
  // ... 同 DestinyCard
  events?: RewardEvent[];
}
```

---

### 3. 获取资料完整度详情

**接口**：`GET /api/user/completeness`

**请求头**：
```
Authorization: Bearer <token>
```

**响应示例**：
```json
{
  "success": true,
  "data": {
    "completeness": 60,
    "breakdown": {
      "birthData": {
        "filled": true,
        "score": 40,
        "maxScore": 40
      },
      "mbti": {
        "filled": true,
        "score": 10,
        "maxScore": 10
      },
      "profession": {
        "filled": true,
        "score": 10,
        "maxScore": 10
      },
      "currentStatus": {
        "filled": false,
        "score": 0,
        "maxScore": 20
      },
      "wishes": {
        "filled": false,
        "score": 0,
        "maxScore": 20
      }
    },
    "nextRewardThreshold": 70
  }
}
```

**TypeScript 类型**：
```typescript
interface CompletenessResult {
  completeness: number;        // 0-100
  breakdown: {
    birthData: FieldScore;
    mbti: FieldScore;
    profession: FieldScore;
    currentStatus: FieldScore;
    wishes: FieldScore;
  };
  nextRewardThreshold?: number;  // 下一个奖励阈值
}

interface FieldScore {
  filled: boolean;
  score: number;
  maxScore: number;
}
```

**完整度计算规则**：
- 基石层（40分）：生辰信息
- 显性层（60分）：
  - MBTI（10分）
  - 职业（10分）
  - 现状描述（20分）
  - 愿景目标（20分）

---

### 4. 同步生辰信息到用户上下文

**接口**：`POST /api/user/sync-birthday-to-context`

**请求头**：
```
Authorization: Bearer <token>
Content-Type: application/json
```

**请求体**：
```json
{
  "birthDate": "1990-01-15",      // 必填，格式：YYYY-MM-DD
  "birthTime": "14:30:00",        // 可选，格式：HH:mm:ss
  "birthLocation": "北京市",       // 可选
  "gender": "male"                // 可选：male, female, other
}
```

**响应示例**：
```json
{
  "success": true,
  "message": "生辰信息已同步到命主名刺",
  "data": {
    "synced": true,
    "userContextUpdated": true
  }
}
```

**注意事项**：
- `birthDate` 为必填字段
- 同步后会自动更新 `preferences.userContext.birthDate`
- 如果数据被锁定，会返回 400 错误，提示稍后重试

---

### 5. 获取隐性信息

**接口**：`GET /api/user/implicit-traits`

**请求头**：
```
Authorization: Bearer <token>
```

**响应示例**：
```json
{
  "success": true,
  "data": {
    "inferred_roles": ["parent", "spouse"],
    "interest_tags": ["wealth", "career", "relationship"],
    "risk_tolerance": "medium",
    "interaction_style": "detailed",
    "last_active_topic": "career",
    "family_structure": {
      "has_spouse": true,
      "has_children": true,
      "children_count": 1
    },
    "profession_hints": ["tech", "finance"]
  }
}
```

**TypeScript 类型**：
```typescript
interface ImplicitTraits {
  inferred_roles?: string[];
  interest_tags?: string[];
  risk_tolerance?: 'low' | 'medium' | 'high';
  interaction_style?: 'concise' | 'detailed';
  last_active_topic?: string;
  family_structure?: {
    has_spouse?: boolean;
    has_children?: boolean;
    children_count?: number;
  };
  profession_hints?: string[];
}
```

**注意事项**：
- 隐性信息由系统自动提取，用户不能直接修改
- 如果用户没有隐性信息，返回空对象 `{}`

---

### 6. 更新隐性信息（内部接口）

**接口**：`POST /api/user/implicit-traits`

**说明**：此接口主要用于系统内部调用，前端通常不需要直接调用。

**请求头**：
```
Authorization: Bearer <token>
Content-Type: application/json
```

**请求体**：
```json
{
  "inferred_roles": ["parent"],
  "interest_tags": ["tech", "finance"]
}
```

**响应示例**：
```json
{
  "success": true,
  "message": "隐性信息已更新",
  "data": {
    "inferred_roles": ["parent"],
    "interest_tags": ["tech", "finance"]
  }
}
```

**注意事项**：
- 数组字段会自动合并并去重
- 对象字段会深度合并
- 标量字段会被新值覆盖

---

### 7. 删除隐性信息

**接口**：`DELETE /api/user/implicit-traits`

**请求头**：
```
Authorization: Bearer <token>
Content-Type: application/json
```

**请求体**（可选）：
```json
{
  "fields": ["inferred_roles", "interest_tags"]
}
```

**响应示例**：
```json
{
  "success": true,
  "message": "已删除指定的隐性信息",
  "data": {
    "deleted": ["inferred_roles", "interest_tags"]
  }
}
```

**注意事项**：
- 如果不提供 `fields` 参数，将删除所有隐性信息
- 删除操作不可恢复

---

### 8. 获取用户资料（扩展）

**接口**：`GET /api/user/profile`

**说明**：此接口已扩展，现在返回 `completeness` 字段。

**响应示例**：
```json
{
  "success": true,
  "data": {
    "id": "uuid",
    "email": "user@example.com",
    "username": "username",
    "completeness": 60,
    "preferences": {
      "userContext": {
        "mbti": "INTJ",
        "profession": "软件工程师"
      }
    },
    "implicit_traits": {
      "interest_tags": ["tech"]
    }
  }
}
```

---

## 数据类型定义

### 完整类型定义

```typescript
// 用户上下文数据（显性层）
interface UserContextData {
  mbti?: string;
  currentStatus?: string;
  identity?: string;
  profession?: string;
  wishes?: string[];
  energyLevel?: 'strong' | 'weak' | 'balanced';
  birthDate?: string;  // ISO 8601 日期字符串
}

// 隐性特征（隐性层）
interface ImplicitTraits {
  inferred_roles?: string[];
  interest_tags?: string[];
  risk_tolerance?: 'low' | 'medium' | 'high';
  interaction_style?: 'concise' | 'detailed';
  last_active_topic?: string;
  family_structure?: {
    has_spouse?: boolean;
    has_children?: boolean;
    children_count?: number;
  };
  profession_hints?: string[];
}

// 奖励事件
interface RewardEvent {
  type: 'COIN_GRANTED' | 'THRESHOLD_REACHED' | 'COMPLETENESS_INCREASED';
  coins?: number;
  reason: string;
  field?: string;
  threshold?: number;
}

// 完整度详情
interface CompletenessResult {
  completeness: number;
  breakdown: {
    birthData: FieldScore;
    mbti: FieldScore;
    profession: FieldScore;
    currentStatus: FieldScore;
    wishes: FieldScore;
  };
  nextRewardThreshold?: number;
}

interface FieldScore {
  filled: boolean;
  score: number;
  maxScore: number;
}
```

---

## 使用示例

### 示例 1：获取并显示命主名刺

```typescript
async function loadDestinyCard(token: string) {
  const response = await fetch('/api/user/destiny-card', {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });

  if (!response.ok) {
    throw new Error('获取命主名刺失败');
  }

  const result = await response.json();
  if (result.success) {
    return result.data;
  }
  throw new Error(result.message || '获取失败');
}

// 使用
const card = await loadDestinyCard(token);
console.log(`完整度：${card.completeness}%`);
console.log(`MBTI：${card.mbti || '未填写'}`);
```

---

### 示例 2：更新命主名刺并处理奖励

```typescript
async function updateDestinyCard(
  token: string,
  data: UpdateDestinyCardRequest
) {
  const response = await fetch('/api/user/destiny-card', {
    method: 'PUT',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(data)
  });

  if (!response.ok) {
    throw new Error('更新失败');
  }

  const result = await response.json();
  if (result.success) {
    // 检查是否有奖励事件
    if (result.data.events && result.data.events.length > 0) {
      result.data.events.forEach((event: RewardEvent) => {
        if (event.type === 'COIN_GRANTED' && event.coins) {
          // 显示奖励动画
          showRewardAnimation(event.coins, event.reason);
        }
      });
    }
    return result.data;
  }
  throw new Error(result.message || '更新失败');
}

// 使用
const updated = await updateDestinyCard(token, {
  mbti: 'INTJ',
  profession: '软件工程师'
});
```

---

### 示例 3：显示完整度进度条

```typescript
async function loadCompleteness(token: string) {
  const response = await fetch('/api/user/completeness', {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });

  const result = await response.json();
  if (result.success) {
    const { completeness, breakdown, nextRewardThreshold } = result.data;
    
    // 显示总体完整度
    updateProgressBar(completeness);
    
    // 显示各字段状态
    Object.entries(breakdown).forEach(([field, score]) => {
      if (!score.filled) {
        showFieldHint(field, `填写此字段可获得 ${score.maxScore} 分`);
      }
    });
    
    // 显示下一个奖励阈值
    if (nextRewardThreshold) {
      showNextRewardHint(nextRewardThreshold);
    }
  }
}
```

---

### 示例 4：同步生辰信息

```typescript
async function syncBirthday(
  token: string,
  birthDate: string,
  gender?: string
) {
  const response = await fetch('/api/user/sync-birthday-to-context', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({
      birthDate,
      gender
    })
  });

  const result = await response.json();
  if (result.success) {
    return result.data;
  }
  
  // 处理错误
  if (result.error === 'INVALID_PARAMETER') {
    throw new Error('日期格式错误，请使用 YYYY-MM-DD 格式');
  }
  throw new Error(result.message || '同步失败');
}
```

---

## 注意事项

### 1. 参数命名兼容性

后端同时支持 `camelCase` 和 `snake_case` 参数名，但建议前端统一使用 `camelCase`：

```typescript
// ✅ 推荐
{ birthDate: '1990-01-15' }

// ✅ 也支持
{ birth_date: '1990-01-15' }
```

### 2. 奖励事件处理

更新命主名刺后，务必检查 `events` 数组：

```typescript
if (result.data.events) {
  result.data.events.forEach(event => {
    // 根据事件类型显示不同的 UI 反馈
    switch (event.type) {
      case 'COIN_GRANTED':
        showCoinReward(event.coins);
        break;
      case 'THRESHOLD_REACHED':
        showThresholdReward(event.threshold, event.coins);
        break;
      case 'COMPLETENESS_INCREASED':
        showCompletenessIncrease();
        break;
    }
  });
}
```

### 3. 完整度计算

完整度计算规则：
- **基石层（40分）**：生辰信息
- **显性层（60分）**：
  - MBTI：10分
  - 职业：10分
  - 现状描述：20分
  - 愿景目标：20分

前端应确保计算逻辑与后端一致。

### 4. 幂等性

- 奖励发放具有幂等性，重复更新同一字段不会重复发放奖励
- 前端可以安全地重试更新操作

### 5. 错误处理

```typescript
try {
  const result = await updateDestinyCard(token, data);
} catch (error) {
  if (error.message.includes('锁定')) {
    // 数据被锁定，提示用户稍后重试
    showError('数据正在处理中，请稍后重试');
  } else if (error.message.includes('认证')) {
    // Token 过期，重新登录
    redirectToLogin();
  } else {
    // 其他错误
    showError(error.message);
  }
}
```

### 6. 隐性信息

- 隐性信息由系统自动提取，用户不能直接修改
- 如果用户没有隐性信息，`GET /api/user/implicit-traits` 返回空对象 `{}`
- 前端应优雅处理空数据情况

---

## 常见问题

### Q1: 更新命主名刺后，为什么没有收到奖励？

**A**: 奖励只在**首次**填写字段时发放。如果字段已经有值，更新不会触发奖励。检查 `events` 数组是否为空。

### Q2: 完整度计算不准确？

**A**: 确保：
1. 生辰信息格式正确（YYYY-MM-DD）
2. 字段值不为空字符串
3. `wishes` 数组不为空

### Q3: 同步生辰信息失败？

**A**: 检查：
1. `birthDate` 格式是否为 `YYYY-MM-DD`
2. 是否提供了必填字段
3. 如果返回"数据被锁定"，等待几秒后重试

### Q4: 如何判断用户是否填写了某个字段？

**A**: 使用 `GET /api/user/completeness` 接口，检查 `breakdown` 中对应字段的 `filled` 属性。

### Q5: 奖励事件什么时候返回？

**A**: 只在更新命主名刺时返回。如果：
- 首次填写字段 → 返回字段奖励事件
- 达到阈值 → 返回阈值奖励事件
- 完整度提升 → 返回完整度提升事件

---

## 相关资源

- **Swagger API 文档**：`http://localhost:3000/api-docs`
- **后端开发文档**：`docs/memory-bank/用户数字孪生系统-后端开发指南.md`
- **API 接口规范**：`docs/memory-bank/用户数字孪生系统-API接口规范.md`

---

**文档最后更新**：2026-01-31  
**如有问题，请联系后端开发团队**
