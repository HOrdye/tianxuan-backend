...
export async function completeTask(
  ...
  try {
    ...
    // 返回结果
    sendSuccess(res, result, result.already_completed ? '任务已完成' : '任务已完成');  // ✅ 统一使用 snake_case
  } catch (error: any) {
...
export async function claimTaskReward(
  ...
  try {
    ...
    // 返回结果
    if (result.already_claimed) {  // ✅ 统一使用 snake_case
      sendSuccess(res, { coins_granted: result.coins_granted }, '奖励已领取');  // ✅ 统一使用 snake_case
    } else {
      sendSuccess(res, { coins_granted: result.coins_granted }, '奖励已领取');  // ✅ 统一使用 snake_case
    }
  } catch (error: any) {
