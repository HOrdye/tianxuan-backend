...
export async function calibrate(
  ...
  try {
    ...
    // 返回成功结果（✅ 统一使用 snake_case）
    sendSuccess(res, {
      record_id: recordId,              // ✅ 统一使用 snake_case
      resonance_params: resonanceParams,  // ✅ 统一使用 snake_case
      manifestation_data: manifestationData,  // ✅ 统一使用 snake_case
    }, '定念成功，共振参数已生成');
  } catch (error: any) {
...
export async function getManifestation(
  ...
  try {
    ...
    if (record.user_id !== userId) {  // ✅ 统一使用 snake_case
    ...
    // 返回显化数据
    sendSuccess(res, {
      record_id: record.id,              // ✅ 统一使用 snake_case
      resonance_date: record.resonance_date,  // ✅ 统一使用 snake_case
      manifestation_data: record.manifestation_data,  // ✅ 统一使用 snake_case
      is_decoded: record.is_decoded,      // ✅ 统一使用 snake_case
    });
  } catch (error: any) {
...
export async function decode(
  ...
  try {
    ...
    // 检查是否已解码
    if (record.is_decoded) {  // ✅ 统一使用 snake_case
      // 如果已解码，直接返回解码数据
      sendSuccess(res, {
        record_id: record.id,              // ✅ 统一使用 snake_case
        decoding_data: record.decoding_data,  // ✅ 统一使用 snake_case
      }, '解码数据已存在');
      return;
    }
    ...
    sendSuccess(res, {
      record_id: record.id,              // ✅ 统一使用 snake_case
      decoding_data: decodingData,      // ✅ 统一使用 snake_case
    }, '解码成功');
  } catch (error: any) {
