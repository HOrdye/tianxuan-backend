...
export async function login(req: Request, res: Response): Promise<void> {
  try {
    console.log(`[Login Controller] 收到登录请求`, {
      body: req.body,
      headers: req.headers,
      method: req.method,
      path: req.path,
    ...
    // 检查请求体是否存在
    if (!req.body) {
      console.error('[Login Controller] 请求体为空');
      sendBadRequest(res, '请求体不能为空');
      return;
    }
    ...
    console.error('[Login Controller] 登录失败:', {
    ...
    // 如果响应头已发送，不再尝试发送错误响应
    if (res.headersSent) {
      console.error('[Login Controller] 响应头已发送，无法发送错误响应');
      return;
    }
    ...
    // 处理其他错误
    sendInternalError(res, '登录失败', error);
  }
}
...
export async function getCurrentUser(
  ...
  try {
    ...
    console.log('[GET /api/auth/me] 原始返回 data 的顶层字段:', Object.keys(profile as object));
    console.log('[GET /api/auth/me] id/userId/user_id:', {
...
export async function mergeGuestData(
  ...
  try {
    ...
    const { guestUserId, profileId, chartId, sessionIds } = req.body;

    if (!guestUserId) {
      sendBadRequest(res, '游客用户ID不能为空');
      return;
    }
    ...
    if (!uuidRegex.test(guestUserId)) {
      sendBadRequest(res, '无效的游客用户ID格式');
      return;
    }

    const { mergeGuestData: mergeGuestDataService } = await import('../services/auth.service');
    const result = await mergeGuestDataService(targetUserId, guestUserId, {
      profileId,
      chartId,
      sessionIds: Array.isArray(sessionIds) ? sessionIds : undefined,
    });

    sendSuccess(res, result, '合并成功');
  } catch (error: any) {
    console.error('合并游客数据失败:', error);

    if (
      error.message === '目标用户不存在' ||
      error.message === '游客用户不存在'
    ) {
