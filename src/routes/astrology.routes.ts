...
import {
  ...
  deleteCompatibility,
  createAnalyzeTask,
  analyzeCompatibilityStream,
  streamCompatibilityAnalysis,
} from '../controllers/astrology.controller';
import { submitInquiry } from '../controllers/strategy.controller';
import { authenticateToken, tokenFromQuery } from '../middleware/auth.middleware';
 ...
 *         "name": "存档名称",
 *         "relationship_type": "self",    // ✅ 统一使用 snake_case
 *         "custom_label": "自定义标签",    // ✅ 统一使用 snake_case
 *         "birth_info": { ... },          // ✅ 统一使用 snake_case，只包含出生信息，不包含完整命盘
 *         "notes": "备注",
 *         "tags": ["标签1", "标签2"],
 *         "created_at": "2025-01-08T12:00:00Z",  // ✅ 统一使用 snake_case
 *         "updated_at": "2025-01-08T12:00:00Z"   // ✅ 统一使用 snake_case
 *       }
 *     ]
 *   }
 * }
 ...
 *   "name": "存档名称",                // 必填
 *   "relationship_type": RelationshipType,  // ✅ 统一使用 snake_case（必填，同时支持 relationshipType）
 *   "custom_label"?: "自定义标签",       // ✅ 统一使用 snake_case（可选，同时支持 customLabel）
 *   "notes"?: "备注",                  // 可选
 *   "tags"?: ["标签1", "标签2"]        // 可选
 * }
 * 
 * ⚠️ 重要：
 * - 如果 relationship_type === 'self'，每个用户只能有一个，创建时会自动更新现有记录
 ...
 *   "data": {
 *     "archive_id": "uuid"  // ✅ 统一使用 snake_case
 *   }
 * }
 */
 ...
 * {
 *   "name"?: string,
 *   "relationship_type"?: RelationshipType,  // ✅ 统一使用 snake_case（同时支持 relationshipType）
 ...
 *   "success": true,
 *   "message": "命盘存档更新成功",
 *   "data": ChartArchive  // 更新后的完整存档数据（字段统一使用 snake_case）
 * }
 */
 ...
 * 请求体：
 * {
 *   "profileId": "uuid",        // 命盘ID（必填，支持 camelCase）
 *   "profile_id": "uuid",       // 命盘ID（可选，支持 snake_case）
 *   "sessionData": { ... }      // 分析会话数据（必填，支持 camelCase）
 *   "session_data": { ... }     // 分析会话数据（可选，支持 snake_case）
 * }
 * 
 * 响应：
 * {
...
router.post('/inquiry', authenticateToken, submitInquiry);

router.post('/compatibility', authenticateToken, createCompatibility);
router.post('/compatibility/analyze/task', authenticateToken, createAnalyzeTask);
router.get('/compatibility/analyze/stream', tokenFromQuery, authenticateToken, streamCompatibilityAnalysis);
router.post('/compatibility/analyze', authenticateToken, analyzeCompatibilityStream);
router.get('/compatibility', authenticateToken, getCompatibilityList);
router.get('/compatibility/:id', authenticateToken, getCompatibilityById);
router.delete('/compatibility/:id', authenticateToken, deleteCompatibility);
