...
export async function completeTask(
  ...
  try {
    ...
    if (existingResult.rows.length > 0) {
      ...
      if (existing.task_status === 'completed' || existing.task_status === 'claimed') {
        return {
          ...
          already_completed: true,        // ✅ 统一使用 snake_case
        };
      }
    }
    ...
    return {
      ...
      already_completed: false,           // ✅ 统一使用 snake_case
    };
  } catch (error: any) {
...
export async function claimTaskReward(
  ...
  try {
    ...
    if (task.task_status === 'claimed') {
      await client.query('ROLLBACK');
      return {
        coins_granted: task.coins_rewarded || 0,  // ✅ 统一使用 snake_case
        already_claimed: true,                     // ✅ 统一使用 snake_case
      };
    }
    ...
    await client.query('COMMIT');
      ...
      return {
        coins_granted: reward,           // ✅ 统一使用 snake_case
        already_claimed: false,           // ✅ 统一使用 snake_case
      };
  } catch (error: any) {
    // 回滚事务
    await client.query('ROLLBACK');
