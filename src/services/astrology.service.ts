...
export interface ChartArchive {
  id: string;
  userId: string;                      // 前端使用 camelCase
  chart: any;                          // 完整命盘数据（ZiweiChart）
  name: string;
  relationshipType: RelationshipType;   // ⚠️ 必填
  customLabel?: string;
  notes?: string;
  tags?: string[];
...
export interface ChartArchiveSummary {
  ...
  customLabel?: string;
  birthInfo: any;                      // ⚠️ 只包含出生信息，不包含完整命盘
  notes?: string;
  tags?: string[];
  createdAt: Date | string;
  updatedAt: Date | string;
...
export interface SaveArchiveResult {
  ...
  message?: string;
  error?: string;
  archiveId?: string;                  // 前端使用 camelCase
}
 ...
 * @returns Promise<ChartArchiveSummary[]> 存档摘要列表
 * 
 * @throws Error 如果查询失败
 */
export async function getChartArchives(
  ...
  try {
    ...
    if (!tableExists.rows[0].exists) {
      console.warn('ziwei_chart_archives 表不存在，返回空数组');
      return [];
    }
