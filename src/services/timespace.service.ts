...
export type { AIGuidanceResponse, TimeDimension, AIGuidanceData };

/**
 * 获取 AI 指导（按维度生成不同格式的运势分析）
 * @param dimension 维度：yearly / monthly / daily
 * @param context 前端提供的时空上下文
 * @returns Promise<AIGuidanceResponse> 平铺的结构化 AI 指导
 */
export async function getAIGuidance(dimension: TimeDimension, context: any): Promise<AIGuidanceResponse> {
  const buildPrompt = PromptBuilderMap[dimension];
  const schema = OutputSchemaMap[dimension];
  const fallback = FallbackMap[dimension];

  const prompt = buildPrompt(context);

  const apiKey = process.env.DEEPSEEK_API_KEY;
  if (!apiKey) {
    throw new Error('DEEPSEEK_API_KEY is not configured in .env file');
  }

  const baseUrl = process.env.DEEPSEEK_BASE_URL || 'https://api.deepseek.com';
  const model = process.env.DEEPSEEK_MODEL || 'deepseek-chat';

  // 流年/流月内容更丰富，需要更多 token
  const maxTokens = dimension === 'daily' ? 1024 : 4096;
  ...
  try {
    ...
    llmOutput = response.data.choices[0].message.content;
    tokensUsed = response.data.usage?.total_tokens || 0;
  } catch (apiError: any) {
    console.error(`[getAIGuidance][${dimension}] DeepSeek API 调用失败:`, apiError.response ? apiError.response.data : apiError.message);
    return {
      dimension,
      data: fallback,
  ...
  try {
    ...
    const validated = schema.parse(parsedOutput) as AIGuidanceData;
    ...
    console.error(`[getAIGuidance][${dimension}] LLM output validation failed:`, error?.message || error);
    if (llmOutput) {
      console.error(`[getAIGuidance][${dimension}] Raw output:`, llmOutput.substring(0, 500));
    }
    return {
      dimension,
      data: fallback,
