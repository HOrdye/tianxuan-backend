...
主星：${record.ziwei_liuday_data.mainStar}
四化：${record.ziwei_liuday_data.transformations.join(', ')}
五行：${record.ziwei_liuday_data.element}
状态：${record.ziwei_liuday_data.state}
核心字：${record.manifestation_data.coreWord}
关键词：${record.manifestation_data.keywords.join(', ')}
请用专业但易懂的语言解释今日气象的含义。`,
    temperature: 0.7,
  });
...
export async function getRecord(recordId: string): Promise<CelestialResonanceRecord | null> {
  try {
    ...
    return {
      ...
      resonance_date: row.resonance_date.toISOString().split('T')[0],
      calibration_data: typeof row.calibration_data === 'string' 
        ? JSON.parse(row.calibration_data) 
        : row.calibration_data,
      resonance_params: typeof row.resonance_params === 'string'
        ? JSON.parse(row.resonance_params)
        : row.resonance_params,
      manifestation_data: typeof row.manifestation_data === 'string'
        ? JSON.parse(row.manifestation_data)
        : row.manifestation_data,
      ziwei_liuday_data: typeof row.ziwei_liuday_data === 'string'
        ? JSON.parse(row.ziwei_liuday_data)
        : row.ziwei_liuday_data,
      decoding_data: row.decoding_data 
        ? (typeof row.decoding_data === 'string' 
            ? JSON.parse(row.decoding_data) 
            : row.decoding_data)
        : undefined,
      is_decoded: row.is_decoded || false,
      decoded_at: row.decoded_at ? row.decoded_at.toISOString() : undefined,
...
function extractAstrologicalReason(record: CelestialResonanceRecord): string {
  const { ziwei_liuday_data, manifestation_data } = record;
  
  return `因流日${ziwei_liuday_data.mainStar}${ziwei_liuday_data.transformations.join('、')}入命，且定念与${ziwei_liuday_data.element}气相合。此象主：${manifestation_data.coreWord}。`;
}
...
async function generateWarningsAndSuggestions(
  ...
  const { ziwei_liuday_data } = record;
  
  // 根据四化生成警告
  const warnings: string[] = [];
  if (ziwei_liuday_data.transformations.includes('化忌')) {
    warnings.push('警惕化忌，午后言多必失，宜闭嘴做事');
  }
  
  // 根据状态生成建议
  const suggestions: string[] = [];
  if (ziwei_liuday_data.state === 'rising') {
    suggestions.push('今日宜主动出击，把握上升机遇');
  } else if (ziwei_liuday_data.state === 'stagnant') {
